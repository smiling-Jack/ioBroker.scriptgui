...
function request(data) {
    if (sim.run_type == "hotrun") {
        _reguest(data)
        send("log", "<b style='color: blue'>Request: </b>" + data + "");
    } else {
        send("log", "<b style='color: blue'>Request: </b>" + data + "");
    }
}
...
function pushover(data) {
    send("log", "<b style='color: blue'>Pushover: </b>" + data.message + "");
}

function email(data) {
    send("log", "<b style='color: blue'>E-Mail: </b>" + data.to + '<br>' + data.subject + '<br>' + data.text + "");
}

function simout(key, data) {
    send("simout", key, data);
}
    ...
    if (data[0] == "run") {
        ...
        for (var i = 0; i < length; i++) {
            ...
            if (patternMatching(eventObj, sim.subscribers[i].pattern)) {
                send("trigger_highlight", sim.subscribers[i].mbs_nr);
...
var setTimeout = function(code, delay) {
    var retval = oldSetTimeout(code, delay);
   timeoutList.push(retval);
    return retval;
};
var clearTimeout = function(id) {
    var ind = timeoutList.indexOf(id);
    if(ind >= 0) {
        timeoutList.splice(ind, 1);
    }
    var retval = oldClearTimeout(id);
    return retval;
};
var setInterval = function(code, delay) {
    var retval = oldSetInterval(code, delay);
   intervalList.push(retval);
    return retval;
};
var clearInterval = function(id) {
    var ind = intervalList.indexOf(id);
    if(ind >= 0) {
        intervalList.splice(ind, 1);
    }
    var retval = oldClearInterval(id);
    return retval;
};
var clearAllTimeouts = function() {
    for(var i in timeoutList) {
        oldClearTimeout(timeoutList[i]);
    }
    timeoutList = new Array();
};
var clearAllIntervals = function() {
    for(var i in intervalList) {
        oldClearInterval(intervalList[i]);
    }
