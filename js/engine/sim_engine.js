...
function run(script) {
    var adapter = {
        ...
        setForeignState: function (id, state, callback) {
        },
        setState: function (id, state, callback) {
        },
        setObject: function (id, obj, callback) {
        },
        setForeignObject: function (idActive, objects, callback) {
        },
        getObject: function (id, callback) {
            callback(false, {})
        },
        delObject: function (id, callback) {
            callback(false)
        },
        delState: function (id, callback) {
            callback()
        },
        ...
        getObjectList: function (obj, callback) {
            callback(err, res)
        },
        extendForeignObject: function (scriptName, obj, callback) {
        },
    ...
    function exec(cmd, callback) {
        if (verbose) log('exec: ' + cmd, 'info');
        if (debug) {
            log(words._('Command %s was not executed, while debug mode is active', cmd), 'warn');
            if (typeof callback === 'function') {
                setTimeout(function () {
                    callback();
                }, 0);
    ...
    function email(msg) {
        if (verbose) log('email(msg=' + JSON.stringify(msg) + ')', 'info');
        adapter.sendTo('email', msg);
    };
    function pushover(msg) {
        if (verbose) log('pushover(msg=' + JSON.stringify(msg) + ')', 'info');
    ...
    function getSubscriptions() {
        ...
        if (verbose) log('getSubscriptions() => ' + JSON.stringify(result), 'info');
        return result;
    };
    function adapterSubscribe(id) {
        ...
        if (objects[_adapter] && objects[_adapter].common && objects[_adapter].common.subscribable) {
            ...
            if (verbose) log('adapterSubscribe: ' + a + ' - ' + id, 'info');
    ...
    function schedule(pattern, callback) {
        ...
        if (pattern.astro) {
            ...
            if (verbose) log('schedule(astro=' + pattern.astro + ', offset=' + pattern.shift + ')', 'info');
            ...
            if (verbose) log('schedule(cron=' + pattern + ')', 'info');
    ...
    function getAstroDate(pattern, date, offsetMinutes) {
        ...
        if (verbose) log('getAstroDate(pattern=' + pattern + ', date=' + date + ') => ' + ts, 'info');
    ...
    function isAstroDay() {
        ...
        if (verbose) log('isAstroDay() => ' + (nowDate >= dayBegin && nowDate <= dayEnd), 'info');
    ...
    function clearSchedule(schedule) {
        for (var i = 0; i < script.schedules.length; i++) {
            if (script.schedules[i] === schedule) {
                ...
                if (verbose) log('clearSchedule() => cleared', 'info');
                return true;
            }
        }
        if (verbose) log('clearSchedule() => invalid handler', 'warn');
        return false;
    };
    function setState(id, state, isAck, callback) {
        ...
        if (states[id]) {
            if (verbose) log('setForeignState(id=' + id + ', state=' + JSON.stringify(state) + ')', 'info');

            if (debug) {
                log('setForeignState(id=' + id + ', state=' + JSON.stringify(state) + ') - ' + words._('was not executed, while debug mode is active'), 'warn');
                ...
                adapter.setForeignState(id, state, function (err) {
                    if (err) log('setForeignState: ' + err, 'error');

                    if (typeof callback === 'function') callback();
                });
            }
        } else if (states[adapter.namespace + '.' + id]) {
            if (verbose) log('setState(id=' + id + ', state=' + JSON.stringify(state) + ')', 'info');

            if (debug) {
                log('setState(' + id + ', ' + JSON.stringify(state) + ') - ' + words._('was not executed, while debug mode is active'), 'warn');
                if (typeof callback === 'function') {
                    setTimeout(function () {
                        callback();
                    }, 0);
            ...
            if (objects[id]) {
                ...
                if (objects[adapter.namespace + '.' + id].type === 'state') {
                    ...
                    if (debug) {
                        log('setState(id=' + id + ', state=' + JSON.stringify(state) + ') - ' + words._('was not executed, while debug mode is active'), 'warn');
                        if (typeof callback === 'function') {
                            setTimeout(function () {
                                callback();
                            }, 0);
    ...
    function setStateDelayed(id, state, isAck, delay, clearRunning, callback) {
        ...
        if (verbose) log('setStateDelayed(id=' + id + ', state=' + state + ', isAck=' + isAck + ', delay=' + delay + ', clearRunning=' + clearRunning + ')', 'info');

        if (clearRunning) {
            if (timers[id]) {
                if (verbose) log('setStateDelayed: clear ' + timers[id].length + ' running timers', 'info');
                ...
                if (verbose) log('setStateDelayed: no running timers', 'info');
            }
        }
    ...
    function clearStateDelayed(id, timerId) {
        if (verbose) log('clearStateDelayed(id=' + id + ', timerId=' + timerId + ')', 'info');
        if (timers[id]) {
            ...
            for (var i = timers[id].length - 1; i >= 0; i--) {
                if (timerId === undefined || timers[id][i].id === timerId) {
                    ...
                    if (verbose) log('clearStateDelayed: clear timer ' + timers[id][i].id, 'info');
                }
            }
    ...
    function getState(id) {
        if (states[id]) {
            if (verbose) log('getState(id=' + id + ', timerId=' + timerId + ') => ' + JSON.stringify(states[id]), 'info');
            return states[id];
        }
        if (states[adapter.namespace + '.' + id]) {
            if (verbose) log('getState(id=' + id + ', timerId=' + timerId + ') => ' + states[adapter.namespace + '.' + id], 'info');
            return states[adapter.namespace + '.' + id];
        }

        if (verbose) log('getState(id=' + id + ', timerId=' + timerId + ') => not found', 'info');

        adapter.log.warn('State "' + id + '" not found');
        return {val: null, notExist: true};
    };
    function getIdByName(name, alwaysArray) {
        if (verbose) log('getIdByName(name=' + name + ', alwaysArray=' + alwaysArray + ') => ' + names[name], 'info');
        if (alwaysArray) {
    ...
    function getObject(id, enumName) {
        if (!objects[id]) {
            if (verbose) log('getObject(id=' + id + ', enumName=' + enumName + ') => does not exist', 'info');
            ...
            if (verbose) log('getObject(id=' + id + ', enumName=' + enumName + ') => ' + JSON.stringify(obj), 'info');
            ...
            if (verbose) log('getObject(id=' + id + ', enumName=' + enumName + ') => ' + JSON.stringify(result), 'info');
            return result;
        }
    };
    ...
    function getEnums(enumName) {
        ...
        if (verbose) log('getEnums(enumName=' + enumName + ') => ' + JSON.stringify(result), 'info');
    ...
    function createState(name, initValue, forceCreation, common, native, callback) {
        ...
        if (verbose) log('createState(name=' + name + ', initValue=' + initValue + ', forceCreation=' + forceCreation + ', common=' + JSON.stringify(common) + ', native=' + JSON.stringify(native) + ')', 'debug');
    ...
    function deleteState(id, callback) {
        ...
        if (verbose) log('deleteState(id=' + id + ')', 'debug');
    ...
    function sendTo(_adapter, cmd, msg, callback) {
        if (verbose) log('sendTo(adapter=' + _adapter + ', cmd=' + cmd + ', msg=' + JSON.stringify(msg) + ')', 'info');
    ...
    function cb(callback) {
        return function () {
            if (scripts[name] && scripts[name]._id === _id) {
    ...
    function compareTime(startTime, endTime, operation, time) {
    ...
    function onStop(cb, timeout) {
        if (verbose) log('onStop(timeout=' + timeout + ')', 'info');
    ...
    function writeFile(_adapter, fileName, data, callback) {
        ...
        if (debug) {
            log('readFile(adapter=' + _adapter + ', fileName=' + fileName + ') - ' + words._('was not executed, while debug mode is active'), 'warn');
            if (typeof callback === 'function') {
                setTimeout(function () {
                    callback();
                }, 0);
            ...
            if (verbose) log('readFile(adapter=' + _adapter + ', fileName=' + fileName + ')', 'info');
            adapter.writeFile(_adapter, fileName, data, callback);
        }
    };
    function readFile(_adapter, fileName, callback) {
    ...
    function getHistory(instance, options, callback) {
        ...
        if (verbose) log('getHistory(instance=' + instance + ', options=' + JSON.stringify(options) + ')', 'debug');
        ...
        var timeout = setTimeout(function () {
            ...
            if (verbose) log('getHistory => timeout', 'debug');
        ...
        adapter.sendTo(instance, 'getHistory', {id: options.id, options: options}, function (result) {
            ...
            if (verbose && result.error)  log('getHistory => ' + result.error, 'error');
            if (verbose && result.result) log('getHistory => ' + result.result.length + ' items', 'debug');

            if (callback) callback(result.error, result.result, options, instance);
            callback = null;
        });
    };
    function runScript(scriptName, isStart) {
        ...
        if (!objects[scriptName] || !objects[scriptName].common) {
            log('Cannot start "' + scriptName + '", because not found', 'error');
            return false;
        } else {
            if (debug) {
                log('runScript(scriptName=' + scriptName + ', isStart=' + isStart + ') - ' + words._('was not executed, while debug mode is active'), 'warn');
            } else {
    ...
    function isScriptActive(scriptName) {
        if (!scriptName.match(/^script\.js\./)) scriptName = 'script.js.' + scriptName;
        if (!objects[scriptName] || !objects[scriptName].common) {
            log('Script does not exist', 'error');
            return false;
        } else {
